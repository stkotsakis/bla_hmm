...
set results=$3 #file used as input for cluster analysis

set clusters_proteins=`cat "$results"_class_"$class"/cluster_proteins_taxa/ASS"$entry_file" | awk '{print $1}' | sort -u`
...
foreach i ($clusters_proteins)
 ...
 foreach j ($assemblies)
  ...
  echo $i $j "$genus" "$family" "$order" "$classt" "$phylum" >> "$results"_class_"$class"/cluster_proteins_taxa/assembly_taxa_proteins_clusters.dat."$entry_file"
 end
end
